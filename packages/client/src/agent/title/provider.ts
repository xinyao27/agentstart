...
import { isOpenCodeNativeTitle } from './opencode'
import { getPiCompatibleSyntheticAgentLabel } from './pi-compatible'
import {
  AGY_AGENT_NAME_RE,
  DROID_AGENT_NAME_RE,
...
function providerTitleLabel(title: string): string | null {
...
// Maps providerTitleLabel()'s product labels to TuiAgent ids — the fallback for
...
export function resolveTerminalTitleAgentType(title: string): TuiAgent | null {
  const label = providerTitleLabel(title)
