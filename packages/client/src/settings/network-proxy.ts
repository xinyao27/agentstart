...
export function normalizeProxyUrl(value: unknown): ProxyUrlValidationResult {
  ...
  if (trimmed.length > PROXY_URL_MAX_LENGTH) {
    return {
      ok: false,
      value: '',
      message: translate('settings.proxy.tooLong', 'Proxy URL is too long.')
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      value: '',
      message: translate('settings.proxy.invalid', 'Enter a valid proxy URL.')
    }
  }
  if (!PROXY_PROTOCOLS.has(parsed.protocol)) {
    return {
      ...
      value: '',
      message: translate(
        'settings.proxy.unsupportedScheme',
  ...
  if (!parsed.hostname) {
    return {
      ok: false,
      value: '',
      message: translate('settings.proxy.missingHost', 'Proxy URL must include a host.')
    }
  }
