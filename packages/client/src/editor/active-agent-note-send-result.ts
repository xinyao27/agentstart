...
export function activeAgentNotesSendFailureMessage(
  ...
  switch (status) {
    ...
    case 'partial-submit-failed':
      return options.explicitTarget
        ? 'The notes may already be pasted in the selected terminal, but AgentStart could not submit them.'
        : 'The notes may already be pasted in the active terminal, but AgentStart could not submit them.'
    case 'sent':
      return ''
  }
}
