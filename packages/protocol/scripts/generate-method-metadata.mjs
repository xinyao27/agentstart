...
} from '../typescript/generated/agent_start/protocol/v1/annotations_pb.js'
...
function methodRecord(service, method) {
  ...
  if (!options || !hasExtension(options, methodPolicyExtension)) {
    throw new Error(`${procedure} has no AgentStart method policy`)
  }
  if (!hasExtension(options, methodTransportPolicyExtension)) {
    throw new Error(`${procedure} has no AgentStart method transport policy`)
  }
...
function renderSwift(services, records) {
  ...
  lines.push('public enum AgentStartMethodMetadata {', '  public static let byProcedure = [')
  for (const record of records) {
    const route = routeNames.get(record.route)
    const reconnect = reconnectNames.get(record.reconnect)
    lines.push(`    "${record.procedure}": AgentStartMethodTransportMetadata(`)
    lines.push(`      procedure: "${record.procedure}",`)
