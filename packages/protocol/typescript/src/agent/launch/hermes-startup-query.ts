...
import { quoteStartupArg, tokenizeStartupCommand, type AgentStartupShell } from '../shell-command'
import { buildShellCommandFromArgv } from '../shell-command'

const QUERY_ENV_LIMIT = 24_000
const QUERY_PLACEHOLDER = '__AGENTSTART_HERMES_STARTUP_QUERY__'
const QUERY_ARG_PLACEHOLDER = `--query=${QUERY_PLACEHOLDER}`
const POSIX_QUERY_VARIABLE = '__agentstart_hermes_startup_query'
const POWERSHELL_QUERY_VARIABLE = 'agentstartHermesStartupQuery'
const POWERSHELL_NATIVE_QUERY_VARIABLE = 'agentstartHermesNativeQuery'

export const AGENTSTART_HERMES_STARTUP_QUERY_ENV = 'AGENTSTART_HERMES_STARTUP_QUERY'
...
function stripAgentStartOwnedHermesArgs(args: readonly string[]): string[] {
...
function normalizeHermesArgv(
  ...
  return [
    ...
    ...stripAgentStartOwnedHermesArgs(baseArgsWithoutChat),
    ...stripAgentStartOwnedHermesArgs(configuredArgsWithoutChat),
    '--tui'
  ]
}
...
function buildQueryCommand(argv: string[], shell: AgentStartupShell): string {
  if (shell !== 'posix') {
    ...
    const script = `$${POWERSHELL_QUERY_VARIABLE} = $env:${AGENTSTART_HERMES_STARTUP_QUERY_ENV}; $${POWERSHELL_NATIVE_QUERY_VARIABLE} = $${POWERSHELL_QUERY_VARIABLE} -replace '(\\\\*)"', '$1$1\\"'; Remove-Item Env:${AGENTSTART_HERMES_STARTUP_QUERY_ENV} -ErrorAction SilentlyContinue; ${invocation}`
  ...
  const script = `${POSIX_QUERY_VARIABLE}="\${${AGENTSTART_HERMES_STARTUP_QUERY_ENV}}"; unset ${AGENTSTART_HERMES_STARTUP_QUERY_ENV}; eval "$(printf %b "${encodedInvocation}")"`
...
export function planHermesStartupQuery(args: {
  ...
  const env = { ...args.agentEnv, [AGENTSTART_HERMES_STARTUP_QUERY_ENV]: args.prompt }
