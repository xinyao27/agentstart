...
public struct AgentStatusClient: Sendable {
    ...
    public func inferInterrupt(
        ...
        intent: AgentStart_Runtime_V1_AgentInterruptIntent,
        inputCount: UInt32?,
        options: RuntimeCallOptions = RuntimeCallOptions()
    ) async throws -> Bool {
        var request = AgentStart_Runtime_V1_AgentStatusServiceInferInterruptRequest()
        ...
        let response = try await transport.unary(
            RuntimeUnaryCall(
                procedure: AgentStartRuntimeV1AgentStatusServiceMethods.inferInterrupt,
                payload: try request.serializedData(),
                options: options
            )
        )
        return try AgentStart_Runtime_V1_AgentStatusServiceInferInterruptResponse(
