...
public struct AiVaultClient: Sendable {
    ...
    public func listSessions(
        ...
        var request = AgentStart_Runtime_V1_AiVaultServiceListSessionsRequest()
        if let limit {
            request.limit = limit
        }
        request.force = force
        ...
        let response = try await transport.unary(
            RuntimeUnaryCall(
                procedure: AgentStartRuntimeV1AiVaultServiceMethods.listSessions,
                payload: try request.serializedData(),
                options: options
            )
        )
        return try AgentStart_Runtime_V1_AiVaultServiceListSessionsResponse(serializedBytes: response)
    }
}
