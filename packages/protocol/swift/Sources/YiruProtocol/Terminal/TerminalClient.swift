...
public struct TerminalClient: Sendable {
    ...
    public func list(
        ...
        var request = Yiru_Runtime_V1_TerminalServiceListRequest()
        if let worktree {
        ...
        return try await unary(
            procedure: YiruRuntimeV1TerminalServiceMethods.list,
            request: request,
            response: Yiru_Runtime_V1_TerminalServiceListResponse.self,
            options: options
        )
    }
    ...
    public func close(
        ...
        var request = Yiru_Runtime_V1_TerminalServiceCloseRequest()
        request.terminal = terminal
        return try await unary(
            procedure: YiruRuntimeV1TerminalServiceMethods.close,
            request: request,
            response: Yiru_Runtime_V1_TerminalServiceCloseResponse.self,
            options: options
        )
    }
    ...
    public func focus(
        terminal: String,
        options: RuntimeCallOptions = RuntimeCallOptions()
    ) async throws -> Yiru_Runtime_V1_TerminalServiceFocusResponse {
        var request = Yiru_Runtime_V1_TerminalServiceFocusRequest()
        request.terminal = terminal
        return try await unary(
            procedure: YiruRuntimeV1TerminalServiceMethods.focus,
            request: request,
            response: Yiru_Runtime_V1_TerminalServiceFocusResponse.self,
            options: options
        )
    }
