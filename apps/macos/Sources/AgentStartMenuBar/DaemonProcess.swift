...
actor DaemonProcess {
  ...
  init() {
    executable = Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/agentstart")
  }
...
enum DaemonFailure: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .start: return translate("The daemon could not start. Open AgentStart again to retry.")
    case .timeout: return translate("The daemon did not respond in time.")
    case .command: return translate("The daemon command failed.")
    case .stop:
      return translate(
        "The daemon is still stopping. AgentStart will stay open; try quitting again shortly.")
    }
  }
}
