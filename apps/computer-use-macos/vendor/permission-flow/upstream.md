...
AgentStart vendors the SystemSettingsKit, PermissionFlow, and
...
of the AgentStart build.

The source is kept in Swift 5 language mode so the helper remains buildable by the macOS release
runner's Swift 6 toolchain. AgentStart resolves System Settings through its bundle identifier instead of
the upstream absolute application path. PermissionFlow strings are hosted in AgentStart's localized app
...
navigation, window tracking, launch animation, and native app drag source. AgentStart's own permission
