...
import Foundation
import ImageIO
import AgentStartComputerUseMacOSCore
import ScreenCaptureKit

private let providerName = "agentstart-computer-use-macos"
...
final class Provider {
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrusted() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for AgentStart Computer Use. Run `agentstart computer permissions` or open Settings > Computer Use, grant Accessibility to AgentStart Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for AgentStart Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
        ...
        return Snapshot(
            ...
            treeText: renderTreeText(app: app, title: title, lines: renderer.lines, focused: renderer.focusedSummary),
    ...
    private func pressKey(params: [String: JSONValue]) throws -> [String: Any] {
        let snapshot = try currentKeyboardSnapshot(params: params)
        try requireTargetWindowFocused(snapshot, restoreWindowRequested: params["restoreWindow"]?.bool == true)
        try Input.pressKey(try requiredString(params, "key"))
        return actionMetadata(
            path: "synthetic",
            actionName: "pressKey",
            verification: unverifiedAction(reason: "synthetic_input")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    let permissionHint = visibleWindowCount > 0
        ? " The app has visible windows, so macOS Accessibility may need AgentStart Computer Use toggled off and on again in System Settings."
        : ""
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["AGENTSTART_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private enum Input {
    ...
    static func pressKey(_ key: String) throws {
        ...
        try keyEvent(parsed.keyCode, down: true, flags: flags)
        try keyEvent(parsed.keyCode, down: false, flags: flags)
...
private func isAuthorizedAgentPeer(_ fd: Int32) -> Bool {
    ...
    guard getpeereid(fd, &peerUserId, &peerGroupId) == 0,
          ...
          isAgentStartDaemonExecutable(executablePath),
          processRunsDaemon(pid),
          isSameAgentStartInstallation(executablePath)
    else {
        return false
    }
    return true
...
private func isAgentStartDaemonExecutable(_ path: String) -> Bool {
    let name = URL(fileURLWithPath: path).lastPathComponent
    return name == "agentstart" || name.hasPrefix("agentstart-bun-darwin-")
}
...
private func isSameAgentStartInstallation(_ daemonPath: String) -> Bool {
    let helperPath = Bundle.main.bundlePath
    let daemonDevelopmentMarker = "/apps/daemon/"
    let helperDevelopmentMarker = "/apps/computer-use-macos/"
    if let daemonMarker = daemonPath.range(of: daemonDevelopmentMarker),
       let helperMarker = helperPath.range(of: helperDevelopmentMarker) {
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("AGENTSTART_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
private func runStdio() {
    fputs("AgentStart Computer Use provider must be launched by AgentStart in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: agentstart-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("agentstart-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: agentstart-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
