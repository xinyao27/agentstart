...
final class ComputerUsePermissionWindowController: NSWindowController {
  init(initialPermission: ComputerUsePermission?) {
    ...
    window.title = translate(
      ...
      fallback: "Enable AgentStart Computer Use"
    )
      ...
      Text(
        allPermissionsGranted
          ? translate(
            ...
            fallback: "AgentStart can use local apps when you ask."
          )
          : translate(
            "computer-use.window.enable-description",
            fallback: """
              AgentStart Computer Use needs these permissions to use apps on your Mac.
              These permissions are only used when you ask AgentStart to perform tasks.
              """
          )
      )
      .font(.system(size: 14))
  ...
  private var rowContent: some View {
    HStack(alignment: .center, spacing: 14) {
      ZStack {
        ...
        AgentStartComputerUseIcon(permission.icon, size: 19)
          .foregroundStyle(.secondary)
  ...
  private var permissionControl: some View {
    ...
    case .granted:
      Label {
        ...
        AgentStartComputerUseIcon(.checkmark, size: 16)
      }
    ...
    case .checking:
      Label {
        ...
        AgentStartComputerUseIcon(.pending, size: 16)
      }
      .font(.system(size: 13, weight: .medium))
      .foregroundStyle(.secondary)
...
enum ComputerUsePermission: String, CaseIterable, Identifiable {
  ...
  var icon: AgentStartComputerUseIconID {
    switch self {
    case .accessibility:
      return .accessibility
    case .screenshots:
