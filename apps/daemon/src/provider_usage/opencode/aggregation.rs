...
impl Aggregation {
    pub fn add(&mut self, event: Event, day: String, location: Location) {
        ...
        let usage = json!({"eventCount":1,"inputTokens":t.input_tokens,"cachedInputTokens":t.cached_input_tokens,
            ...
            "totalTokens":t.total_tokens,"estimatedCostUsd":event.cost});
...
fn add_usage(target: &mut Value, source: &Value, keys: &[&str]) {
    let left_tokens = number(target, "totalTokens");
    let right_tokens = number(source, "totalTokens");
    let left = target["estimatedCostUsd"].as_f64();
    let right = source["estimatedCostUsd"].as_f64();
    target["estimatedCostUsd"] = json!(if (left_tokens > 0 && left.is_none())
        || (right_tokens > 0 && right.is_none())
