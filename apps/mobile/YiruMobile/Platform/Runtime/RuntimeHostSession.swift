...
actor RuntimeHostSession {
    ...
    func protocolAccounts() async throws -> Yiru_Runtime_V1_AccountsSnapshot {
        try await runtimeProtocolCall { peer in
    ...
    func protocolRegisterPush(
        ...
        _ = try await runtimeProtocolCall { peer in
            try await peer.runtimeProtocolRegisterPush(registration: registration)
        }
    }
