...
actor RuntimePeer {
    ...
    func runtimeProtocolAccounts() async throws -> Yiru_Runtime_V1_AccountsSnapshot {
        guard !isClosed else { throw RuntimeServiceError.closed }
        ...
        -> AsyncThrowingStream<Yiru_Runtime_V1_AccountsSnapshot, Error>
    {
        ...
        let (stream, continuation) = AsyncThrowingStream.makeStream(
            of: Yiru_Runtime_V1_AccountsSnapshot.self
        )
    ...
    func runtimeProtocolMissedNotifications(after sequence: Int64) async throws
        -> Yiru_Runtime_V1_GetMissedSinceResponse
    {
        guard !isClosed else { throw RuntimeServiceError.closed }
    ...
    func runtimeProtocolRegisterPush(
        registration: NotificationsPushRegistration?
    ) async throws -> Bool {
        guard !isClosed else { throw RuntimeServiceError.closed }
        let response = try await NotificationsClient(transport: protocolAdapter).registerPush(
...
private actor ProtocolNotificationCursor {
    ...
    private func nextResponse() async throws -> Yiru_Runtime_V1_SubscribeResponse? {
...
private actor ProtocolAccountsCursor {
    private var isCancelled = false
    private var readySnapshot: Yiru_Runtime_V1_AccountsSnapshot?
    ...
    func next() async throws -> Yiru_Runtime_V1_AccountsSnapshot? {
    ...
    private func nextResponse() async throws -> Yiru_Runtime_V1_AccountsServiceSubscribeResponse? {
        guard !isCancelled, var iterator else { return nil }
        self.iterator = nil
        do {
            let response = try await iterator.next()
