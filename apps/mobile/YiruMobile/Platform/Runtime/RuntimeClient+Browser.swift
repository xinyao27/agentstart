...
extension RuntimeClient: WorkspaceBrowserRepository {
    ...
    func navigateBrowser(
        ...
        let command: Yiru_Runtime_V1_ExecuteMobileRequest.OneOf_Command =
            switch action {
    ...
    func navigateBrowser(
        ...
        var command = Yiru_Runtime_V1_UrlCommand()
    ...
    func clickBrowser(
        ...
        let target = browserTarget(worktreeID: worktreeID, pageID: pageID)
        do {
            var command = Yiru_Runtime_V1_MouseClickCommand()
    ...
    func scrollBrowser(
        ...
        var command = Yiru_Runtime_V1_MouseWheelCommand()
        command.target = target
        command.dy = deltaY
        command.dx = deltaX
    ...
    func pressBrowserKey(
        ...
        var command = Yiru_Runtime_V1_KeyCommand()
        command.target = browserTarget(worktreeID: worktreeID, pageID: pageID)
        command.key = key
    ...
    func insertBrowserText(
        ...
        var command = Yiru_Runtime_V1_TextCommand()
        command.target = browserTarget(worktreeID: worktreeID, pageID: pageID)
        command.text = text
    ...
    func browserTabCreate(
        ...
        var command = Yiru_Runtime_V1_TabCreateCommand()
    ...
    private func executeMobile(
        hostID: String,
        command: Yiru_Runtime_V1_ExecuteMobileRequest.OneOf_Command
    ) async throws -> Yiru_Runtime_V1_ExecuteMobileResponse.OneOf_Result {
        var request = Yiru_Runtime_V1_ExecuteMobileRequest()
        request.command = command
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: YiruRuntimeV1BrowserHostServiceMethods.executeMobile,
            request: request,
            response: Yiru_Runtime_V1_ExecuteMobileResponse.self
        )
    ...
    private func screencastRequest(
        ...
        pageID: String,
        configuration: WorkspaceBrowserStreamConfiguration
    ) -> Yiru_Runtime_V1_BrowserScreencastSubscribeRequest {
        let scale = min(max(configuration.scale, 1), 2.5)
        let isMobile = configuration.viewMode == .mobile
        var request = Yiru_Runtime_V1_BrowserScreencastSubscribeRequest()
    ...
    private func browserTarget(
        worktreeID: String,
        pageID: String?
    ) -> Yiru_Runtime_V1_BrowserTarget {
        var target = Yiru_Runtime_V1_BrowserTarget()
...
nonisolated private func targetCommand(
    _ target: Yiru_Runtime_V1_BrowserTarget
) -> Yiru_Runtime_V1_TargetCommand {
    var command = Yiru_Runtime_V1_TargetCommand()
    command.target = target
    return command
}
...
nonisolated private func optionalTextCommand(
    _ target: Yiru_Runtime_V1_BrowserTarget
) -> Yiru_Runtime_V1_OptionalTextCommand {
    var command = Yiru_Runtime_V1_OptionalTextCommand()
    command.target = target
    return command
}
...
nonisolated private func moveCommand(
    _ target: Yiru_Runtime_V1_BrowserTarget,
    point: WorkspaceBrowserPoint
) -> Yiru_Runtime_V1_MouseMoveCommand {
    var command = Yiru_Runtime_V1_MouseMoveCommand()
    command.target = target
    command.x = point.x
    command.y = point.y
    return command
...
nonisolated private func buttonCommand(
    _ target: Yiru_Runtime_V1_BrowserTarget,
    button: WorkspaceBrowserButton
) -> Yiru_Runtime_V1_MouseButtonCommand {
    var command = Yiru_Runtime_V1_MouseButtonCommand()
    command.target = target
    command.button = button.rawValue
    return command
}
...
nonisolated private func navigationURL(
    of result: Yiru_Runtime_V1_ExecuteMobileResponse.OneOf_Result
) throws -> String {
    let navigation: Yiru_Runtime_V1_NavigationResult?
    switch result {
...
nonisolated private func browserFrame(
    _ event: Yiru_Runtime_V1_BrowserScreencastFrame
