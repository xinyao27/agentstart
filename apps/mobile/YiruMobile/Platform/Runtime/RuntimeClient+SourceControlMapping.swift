...
nonisolated func sourceEntry(_ entry: Yiru_Runtime_V1_GitStatusEntry) -> SourceFileEntry {
    SourceFileEntry(
        path: entry.path,
...
nonisolated func sourceFileStatus(_ status: Yiru_Runtime_V1_GitChangeStatus) -> SourceFileStatus {
    switch status {
    case .added: .added
...
nonisolated func sourceStagingArea(_ area: Yiru_Runtime_V1_GitStatusArea) -> SourceStagingArea {
    switch area {
    case .staged: .staged
    case .untracked: .untracked
...
nonisolated func sourceBranchFile(_ entry: Yiru_Runtime_V1_GitChangeEntry) -> SourceBranchFile {
    SourceBranchFile(
        path: entry.path,
        status: sourceFileStatus(entry.status),
        oldPath: entry.hasOldPath ? entry.oldPath : nil,
...
nonisolated func sourceCommitFile(_ entry: Yiru_Runtime_V1_GitChangeEntry) -> SourceCommitFile {
    SourceCommitFile(
        path: entry.path,
        status: sourceFileStatus(entry.status),
        oldPath: entry.hasOldPath ? entry.oldPath : nil,
...
nonisolated func sourceConflictOperation(
    _ operation: Yiru_Runtime_V1_GitConflictOperation
...
nonisolated func sourceUpstreamStatus(
    _ upstream: Yiru_Runtime_V1_GitUpstreamStatus
...
nonisolated func sourceCompareStatus(_ status: Yiru_Runtime_V1_GitCompareStatus) -> String {
    switch status {
    case .ready: "ready"
