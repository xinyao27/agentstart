...
nonisolated struct WorkspaceHostedSource: Identifiable, Hashable, Sendable {
    ...
    init(item: AgentStart_Runtime_V1_GitHubWorkItem) {
        self.init(
            id: "github:\(item.id)",
...
nonisolated private func hostedSourceState(_ state: AgentStart_Runtime_V1_GitHubPrState) -> String {
    switch state {
    case .closed: "closed"
    case .merged: "merged"
    case .draft: "draft"
...
nonisolated struct WorkspacePushTarget: Hashable, Sendable {
    ...
    init(pushTarget: AgentStart_Runtime_V1_WorktreePushTarget) {
