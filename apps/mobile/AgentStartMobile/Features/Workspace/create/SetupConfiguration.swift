...
nonisolated enum WorkspaceSetupRunPolicy: String, Sendable {
    ...
    init(protocolValue: AgentStart_Runtime_V1_RepoSetupRunPolicy) throws {
...
nonisolated struct WorkspaceSetupDetails: Hashable, Sendable {
    ...
    init(protocolValue: AgentStart_Runtime_V1_RepoServiceGetHooksResponse) throws {
        let command =
        ...
        case .agentStartYaml: source = "agentstart.yaml"
...
// Why: the UI document keeps trustedAgentStartHooks as an open JSON object, so the
...
nonisolated func workspaceTrustedHooks(
    ...
    guard let repos = fields?["trustedAgentStartHooks"]?.objectValue else { return [:] }
