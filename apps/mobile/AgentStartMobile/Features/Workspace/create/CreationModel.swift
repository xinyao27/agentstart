...
final class WorkspaceCreationModel {
    ...
    func load() async {
        ...
        do {
            ...
            phase = .failed("AgentStart could not load workspace creation options from this host.")
        }
    }
    ...
    func loadSelectedRepoConfiguration() async {
        ...
        do {
            ...
            errorMessage = String(
                localized: "AgentStart could not safely inspect this repository's setup commands."
            )
        }
    }
    ...
    private func create(setupDecision: WorkspaceSetupDecision) async -> WorkspaceSummary? {
        ...
        do {
            ...
            errorMessage = String(
                localized:
                    "The workspace was created, but AgentStart could not open it yet. Refresh the workspace list."
            )
        } catch WorkspaceCreationError.rejected(let message) {
            errorMessage =
                message ?? String(localized: "AgentStart could not create this workspace.")
        } catch {
            errorMessage = String(localized: "AgentStart could not create this workspace.")
        }
        return nil
    }
