...
struct WorkspaceFileExplorerView: View {
            ...
            case .failed(let failure):
                stateView(
                    ...
                    description: failure.isConnectionFailure
                        ...
                        : "AgentStart could not load this file tree."
                )
    ...
    private func fileRow(_ row: WorkspaceFileRow, leftInset: CGFloat) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: Theme.Spacing.small) {
                if row.kind == .directory {
                    AgentStartIcon(isExpanded ? .chevronDown : .chevronRight, size: 16)
                        .frame(width: 16)
                } else {
                    Color.clear.frame(width: 16)
                }
                AgentStartIcon(iconName(row), size: 16)
                    .frame(width: 16)
    ...
    private func iconName(_ row: WorkspaceFileRow) -> AgentStartIconID {
