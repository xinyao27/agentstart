...
nonisolated struct TerminalQuickCommand: Hashable, Identifiable, Sendable {
    ...
    init?(proto: AgentStart_Runtime_V1_SettingsQuickCommand) {
    ...
    var upsert: AgentStart_Runtime_V1_SettingsQuickCommandUpsert {
        var scope = AgentStart_Runtime_V1_SettingsQuickCommandScope()
        switch self.scope {
        case .global:
            scope.global = true
        ...
        var command = AgentStart_Runtime_V1_SettingsQuickCommand()
        command.id = id
        command.label = label
        command.scope = scope
        switch action {
        case .terminal(let commandText, let appendEnter):
            var payload = AgentStart_Runtime_V1_SettingsTerminalCommand()
        ...
        case .agent(let agentID, let prompt):
            var payload = AgentStart_Runtime_V1_SettingsAgentPrompt()
            payload.agent = agentID
            payload.prompt = prompt
            command.kind = .agentPrompt(payload)
        }
        return AgentStart_Runtime_V1_SettingsQuickCommandUpsert.with {
            $0.command = command
        }
    }
}
