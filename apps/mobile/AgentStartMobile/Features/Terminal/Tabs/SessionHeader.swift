...
struct TerminalSessionHeader: View {
    ...
    var body: some View {
        ...
        Text(verbatim: title)
            ...
            // Why: at accessibility sizes the scaled title no longer fits that slot, so it sizes
            // itself and truncates against the bar instead of clipping mid-glyph.
            .frame(maxWidth: dynamicTypeSize.isAccessibilitySize ? nil : 220)
    }
}
