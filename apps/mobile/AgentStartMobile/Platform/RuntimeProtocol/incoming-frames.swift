...
extension RuntimeProtocolAdapter {
    func handleFrame(_ frame: AgentStart_Protocol_V1_Frame) async throws {
    ...
    func acceptPayload(_ payload: AgentStart_Protocol_V1_Payload) async throws {
        guard var call = pending[payload.callID] else {
            if locallyCancelledCalls.contains(payload.callID) { return }
            throw RuntimeTransportError.unexpectedMessage
        }
        guard payload.data.count <= agentstartRuntimeMaxPendingResponseBytes - pendingResponseBytes
        else {
            await cancel(
    ...
    func finishIncomingCall(callID: UInt64, status: AgentStart_Protocol_V1_Status) async throws {
        if await finishCall(callID: callID, status: status) { return }
        guard !isRetiredClientCall(callID) else { return }
        throw RuntimeTransportError.unexpectedMessage
    }

    func finishIncomingCancel(callID: UInt64, status: AgentStart_Protocol_V1_Status) async throws {
    ...
    @discardableResult
    func finishCall(callID: UInt64, status: AgentStart_Protocol_V1_Status) async -> Bool {
