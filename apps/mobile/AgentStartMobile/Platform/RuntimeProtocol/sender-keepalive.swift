...
extension RuntimeProtocolAdapter {
    ...
    func sendFrame(_ body: AgentStart_Protocol_V1_Frame.OneOf_Body) async throws {
        try await sendFrames([body])
    }

    func sendFrames(_ bodies: [AgentStart_Protocol_V1_Frame.OneOf_Body]) async throws {
        ...
        let frames = try bodies.map { body in
            ...
            sequence += 1
            var frame = AgentStart_Protocol_V1_Frame()
            frame.protocolVersion = agentstartRuntimeProtocolVersion
    ...
    func ensureCallAdmission() throws {
        ...
        guard pending.count < agentstartRuntimeMaxPendingCalls else {
    ...
    func reserveRequestBytes(_ byteCount: Int) throws {
        guard byteCount <= agentstartRuntimeMaxPendingRequestBytes - pendingRequestBytes else {
            throw RuntimeTransportError.requestExceedsCredit
        }
    ...
    func timeoutTask(
        ...
        let timeout = options.timeout ?? (unary ? agentstartRuntimeDefaultTimeout : nil)
    ...
    func acceptPong(_ pong: AgentStart_Protocol_V1_Pong) async {
