...
extension RuntimeClient: TerminalQuickCommandRepository {
    ...
    func quickCommands(for hostID: String) async throws -> [TerminalQuickCommand] {
        ...
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1SettingsServiceMethods.getTerminalQuickCommands,
            request: AgentStart_Runtime_V1_SettingsServiceGetTerminalQuickCommandsRequest(),
            response: AgentStart_Runtime_V1_SettingsServiceTerminalQuickCommandsResponse.self
        )
    ...
    func mutateQuickCommands(
        ...
        var mutationProto = AgentStart_Runtime_V1_SettingsQuickCommandMutation()
        switch mutation {
        case .upsert(let command):
            mutationProto.upsert = command.upsert
        case .delete(let id):
            mutationProto.delete = AgentStart_Runtime_V1_SettingsQuickCommandDelete.with {
                $0.id = id
            }
        }
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1SettingsServiceMethods.updateTerminalQuickCommands,
            request: AgentStart_Runtime_V1_SettingsServiceUpdateTerminalQuickCommandsRequest.with {
