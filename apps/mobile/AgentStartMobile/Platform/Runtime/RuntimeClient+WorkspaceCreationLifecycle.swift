...
extension RuntimeClient {
    func createWorkspace(
        ...
        guard
            ...
            throw WorkspaceCreationError.rejected(
                String(localized: "Update AgentStart on this host before creating a workspace.")
            )
        }
        guard
            try await supportsCapability(
    ...
    private func worktreeCreateRequest(
    ...
    ) -> AgentStart_Runtime_V1_WorktreeServiceCreateRequest {
        var request = AgentStart_Runtime_V1_WorktreeServiceCreateRequest()
        ...
        if let pushTarget = draft.pushTarget {
            var target = AgentStart_Runtime_V1_WorktreePushTarget()
        ...
        if let linkedPullRequest = draft.linkedPullRequest {
            var linkedPR = AgentStart_Runtime_V1_WorktreeNullableInt64()
