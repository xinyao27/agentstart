...
nonisolated private let workspaceResumeTimeout: Duration = .seconds(120)
...
extension RuntimeClient: WorkspaceRepository {
    ...
    func activateWorkspace(hostID: String, workspaceID: String) async throws {
        var request = AgentStart_Runtime_V1_WorktreeServiceActivateRequest()
        request.worktree = worktreeSelector(workspaceID)
        request.notifyClients = false
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.activate,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServiceActivateResponse.self,
            timeout: workspaceResumeTimeout
        )
    ...
    func sleepWorkspace(hostID: String, workspaceID: String) async throws {
        var request = AgentStart_Runtime_V1_WorktreeServiceSleepRequest()
        request.worktree = worktreeSelector(workspaceID)
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.sleep,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServiceSleepResponse.self
        )
    ...
    func removeWorkspace(hostID: String, workspaceID: String) async throws {
        let revision = try await workspaceMutationRevision(hostID: hostID, workspaceID: workspaceID)
        var request = AgentStart_Runtime_V1_WorktreeServiceRemoveRequest()
        request.worktree = worktreeSelector(workspaceID)
        request.expectedRevision = Int64(revision)
        request.force = true
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.remove,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServiceRemoveResponse.self
        )
    ...
    func protocolWorktreeShow(
        hostID: String,
        workspaceID: String
    ) async throws -> AgentStart_Runtime_V1_WorktreeServiceShowResponse {
        var request = AgentStart_Runtime_V1_WorktreeServiceShowRequest()
        request.worktree = worktreeSelector(workspaceID)
        return try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.show,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServiceShowResponse.self
        )
    }
    ...
    func protocolSetWorktree(
        ...
        workspaceID: String,
        revision: Int,
        patch: (inout AgentStart_Runtime_V1_WorktreeSetPatch) -> Void
    ) async throws {
        var request = AgentStart_Runtime_V1_WorktreeServiceSetRequest()
        request.worktree = worktreeSelector(workspaceID)
        request.expectedRevision = Int64(revision)
        var value = AgentStart_Runtime_V1_WorktreeSetPatch()
        patch(&value)
        request.patch = value
        _ = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.set,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServiceSetResponse.self
        )
    }

    func fetchWorkspaces(for hostID: String) async throws -> WorkspaceSnapshot {
        async let repos = fetchWorkspaceRepos(for: hostID)
        var request = AgentStart_Runtime_V1_WorktreeServicePsRequest()
        request.limit = 10_000
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1WorktreeServiceMethods.ps,
            request: request,
            response: AgentStart_Runtime_V1_WorktreeServicePsResponse.self
        )
    ...
    func fetchWorkspaceRepos(for hostID: String) async -> [WorkspaceRepo] {
        let response: AgentStart_Runtime_V1_RepoServiceListResponse? = try? await protocolUnary(
            hostID: hostID,
