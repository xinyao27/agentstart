...
extension RuntimeClient: WorkspaceFilesRepository {
    func loadWorkspaceDirectory(
        ...
        do {
            var request = AgentStart_Runtime_V1_FilesServiceReadDirectoryRequest()
            request.worktree = "id:\(worktreeID)"
            request.relativePath = relativePath
            let response = try await protocolUnary(
                hostID: hostID,
                procedure: AgentStartRuntimeV1FilesServiceMethods.readDirectory,
                request: request,
                response: AgentStart_Runtime_V1_FilesServiceReadDirectoryResponse.self
            )
    ...
    func protocolFilesOpen(
        ...
        worktree: String,
        relativePath: String
    ) async throws -> AgentStart_Runtime_V1_FileOpenResult {
        var request = AgentStart_Runtime_V1_FilesServiceOpenRequest()
        request.worktree = worktree
        request.relativePath = relativePath
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1FilesServiceMethods.open,
            request: request,
            response: AgentStart_Runtime_V1_FilesServiceOpenResponse.self
        )
