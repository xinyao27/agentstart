...
extension RuntimeClient: WorkspaceContentRepository {
    func createWorkspaceMarkdown(
        ...
        for attempt in 1...100 {
            ...
            do {
                var request = AgentStart_Runtime_V1_FilesServiceCreateFileRequest()
                request.worktree = worktree
                request.relativePath = relativePath
                let response = try await protocolUnary(
                    hostID: hostID,
                    procedure: AgentStartRuntimeV1FilesServiceMethods.createFile,
                    request: request,
                    response: AgentStart_Runtime_V1_FilesServiceCreateFileResponse.self
                )
    ...
    func readWorkspaceMarkdown(
        ...
        do {
            var request = AgentStart_Runtime_V1_MarkdownServiceReadTabRequest()
            request.worktree = worktreeSelector(worktreeID)
            request.tabID = tab.id
            let response = try await protocolUnary(
                hostID: hostID,
                procedure: AgentStartRuntimeV1MarkdownServiceMethods.readTab,
                request: request,
                response: AgentStart_Runtime_V1_MarkdownServiceReadTabResponse.self
            )
    ...
    func saveWorkspaceMarkdown(
        ...
        var request = AgentStart_Runtime_V1_MarkdownServiceSaveTabRequest()
        request.worktree = worktreeSelector(worktreeID)
        request.tabID = tabID
        ...
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1MarkdownServiceMethods.saveTab,
            request: request,
            response: AgentStart_Runtime_V1_MarkdownServiceSaveTabResponse.self
        )
        return WorkspaceMarkdownDocument(
            content: response.content,
            version: response.version,
    ...
    func readWorkspaceFile(
        ...
        case .image:
            var request = AgentStart_Runtime_V1_FilesServiceReadPreviewRequest()
            request.worktree = worktreeSelector(worktreeID)
            request.relativePath = descriptor.relativePath
            let response = try await protocolUnary(
                hostID: hostID,
                procedure: AgentStartRuntimeV1FilesServiceMethods.readPreview,
                request: request,
                response: AgentStart_Runtime_V1_FilesServiceReadPreviewResponse.self
            )
    ...
    private func readWorkspaceTextFile(
        ...
        worktreeID: String,
        relativePath: String
    ) async throws -> AgentStart_Runtime_V1_FileReadResult {
        var request = AgentStart_Runtime_V1_FilesServiceReadRequest()
        request.worktree = worktreeSelector(worktreeID)
        request.relativePath = relativePath
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1FilesServiceMethods.read,
            request: request,
            response: AgentStart_Runtime_V1_FilesServiceReadResponse.self
        )
...
nonisolated private func workspaceMarkdownReadOnlyReason(
    _ response: AgentStart_Runtime_V1_MarkdownServiceReadTabResponse
