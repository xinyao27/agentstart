    ...
    TerminalSessionRuntime
{
    ...
    func protocolAccounts(hostID: String) async throws -> AgentStart_Runtime_V1_AccountsSnapshot {
        let credential = try await credential(for: hostID)
    ...
    func protocolClientEventsUnsubscribe(
        ...
        var request = AgentStart_Runtime_V1_ClientEventsServiceUnsubscribeRequest()
        request.subscriptionID = subscriptionID
        let response = try await protocolUnary(
            hostID: hostID,
            procedure: AgentStartRuntimeV1ClientEventsServiceMethods.unsubscribe,
            request: request,
            response: AgentStart_Runtime_V1_ClientEventsServiceUnsubscribeResponse.self
        )
