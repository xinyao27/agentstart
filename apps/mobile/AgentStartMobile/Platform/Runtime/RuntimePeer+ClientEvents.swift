    ...
    func clientEventsEvents() async throws -> ClientEventsEventSource {
        ClientEventsEventSource(
            stream: try await protocolAdapter.subscribe(
                RuntimeUnaryCall(
                    procedure: AgentStartRuntimeV1ClientEventsServiceMethods.subscribe,
                    payload: try AgentStart_Runtime_V1_ClientEventsServiceSubscribeRequest()
                        .serializedData(),
...
nonisolated struct ClientEventsEventSource: AsyncSequence, Sendable {
    typealias Element = AgentStart_Runtime_V1_ClientEventsServiceEvent
